...
import { fetchUserOrders } from '../redux/slices/orderSlice'; // Assuming you have an action to fetch orders
...
const MyOrdersPage = () => {
   // const [orders, setOrders] = useState([]);
    ...
    useEffect(() => {
        // Fetch orders from the server or state management
        dispatch(fetchUserOrders());
     },[]); // Assuming you have an action to fetch orders
    ...
    //         setOrders(mockOrders); // ✅ Correctly setting state
                ...
                <table className='min-w-full text-left text-gray-500'>
                    ...
                    <tbody>
                        {orders.length >0 ? (
                            orders.map((order)=>(
                                <tr key={order._id}
                                    onClick={()=>handleRowClick(order._id)} className='border-b hover:border-r-gray-50 cursor-pointer'>
